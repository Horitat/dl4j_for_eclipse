...
public class LayerUpdater implements Updater {
    ...
    public void update(Layer layer, Gradient gradient, int iteration, int miniBatchSize) {
        ...
        for (Map.Entry<String, INDArray> gradientPair : gradient.gradientForVariable().entrySet()) {
            ...
            if (decay != LearningRatePolicy.None || layer.conf().getLayer().getUpdater() == deeplearning4j_nn.src.main.java.org.deeplearning4j.nn.conf.Updater.NESTEROVS)
    ...
    public void applyLrDecayPolicy(LearningRatePolicy decay, Layer layer, int iteration, String variable){
        ...
        if(layer.conf().getLayer().getUpdater() == deeplearning4j_nn.src.main.java.org.deeplearning4j.nn.conf.Updater.NESTEROVS) {
    ...
    public GradientUpdater init(String variable, Layer layer){
        ...
        if(updater == null){
        	deeplearning4j_nn.src.main.java.org.deeplearning4j.nn.conf.Updater u = layer.conf().getLayer().getUpdaterByParam(variable);
            switch (u){
                case SGD:
