...
package deeplearning4j_core.src.test.java.org.deeplearning4j.plot;
...
public class BarnesHutTsneTest {
    ...
    public void testBuilderFields() throws Exception {
        ...
        //self made
        assertEquals("maxIter", maxIter, b.getmaxIter());
        assertEquals(realMin,b.getrealMin(),DELTA);
        assertEquals(initialMomentum,b.getinitialMomentum(),DELTA);
        assertEquals(finalMomentum, b.getfinalMomentum(),DELTA);
        assertEquals(momentum,b.getmomentum(),DELTA);
        assertEquals("switchMomentumnIteration", switchMomentumIteration, b.getswitchMomentumIteration());
        assertEquals("normalize", normalize,b.getnormalize());
        assertEquals("stopLyingInMemoryLookupTable.javaIteration", stopLyingIteration,b.getstopLyingIteration());
        assertEquals(tolerance,b.gettolerance(),DELTA);
        assertEquals(learningRate,b.getlearningRate(),DELTA);
        assertEquals("useAdaGrad", useAdaGrad, b.getuseAdaGrad());
        assertEquals(perplexity,b.getPerplexity(),DELTA);
        assertEquals(minGain,b.getminGain(),DELTA);
